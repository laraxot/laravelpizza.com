...
- **Specialized Head Management with `<x-metatags>`:**
    - The `<x-metatags>` component (e.g., from the `Cms` module) is designed to render the entire HTML `<head>` section, handling all standard meta tags, dynamic titles, and SEO information.
    - Assets like `@vite` and `@livewireStyles` should be passed as slot content to `<x-metatags>` (e.g., `<x-metatags> @vite(...) @livewireStyles </x-metatags>`) to ensure they are correctly placed within the dynamically generated `<head>` tag. This prevents nested `<head>` tags and ensures all meta-information is consolidated.
