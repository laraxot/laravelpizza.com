...
- Regola root modulo: nella root di `laravel/Modules/<Modulo>/` sono vietati file `.txt` (zero tolleranza). Spostare in `docs/raw/root-import/` con `bash bashscripts/tools/fix-module-root-hygiene.sh`. Vedi `docs/wiki/rules/module-root-cleanup-rules.md`.
