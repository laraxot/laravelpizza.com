...
- Regola Laravel Localization: le pagine pubbliche devono essere servite e testate con URL localizzate. I middleware `localeSessionRedirect` e `localizationRedirect` vanno considerati parte dell'architettura SEO e routing, i link/form verso pagine localizzate devono usare gli helper `LaravelLocalization`, e con route tradotte non si usa `route:cache` ma `route:trans:cache`.
- Regola Laravel Folio: il frontoffice pubblico e' `Folio-first`. Il filename Blade e il mount `Folio::path(...)->uri()/domain()->middleware()` definiscono il contratto di routing; `index.blade.php`, segmenti dinamici (`[slug]`, `[...segments]`), implicit model binding, `name()`, `middleware()` e `render()` vanno preferiti a parsing manuale dei path o a controller duplicati. Con Folio la `route:cache` resta una pratica valida; le eccezioni riguardano la localizzazione tradotta gestita da altri package, non Folio in se'.
- Regola temi/traduzioni: nei temi non devono esistere stringhe localizzabili hardcoded. Sono vietati sia testo nudo in lingua sia pseudo-localizzazione tipo `__('Accedi')` o `__('Registrati')`. Nei Blade del tema usare solo chiavi namespaced (`pub_theme::...`, `module::...`) o dati gia' tradotti a monte.
- Regola logging: `Log::info()` e `Log::debug()` non sono ammessi come logging applicativo di routine o debug permanente. Per errori usare `Log::error()`, per anomalie recuperabili `Log::warning()`, per audit trail usare activity log / persistenza dedicata, per monitoring usare strumenti osservabili dedicati.
- Regola Git: workflow rigorosamente forward-only. Su branch condivisi sono vietati `git reset`, `git checkout` di ripristino, force push e anche `git revert` come scorciatoia standard. Gli errori si correggono con nuovi commit espliciti che sistemano il problema senza cancellare o "annullare" la storia.
- Regola root modulo: nella root di `laravel/Modules/<Modulo>/` (e temi) sono vietati file `.txt` (zero tolleranza) e `.md` diversi da `README.md`. Spostare in `docs/raw/root-import/` con `bash bashscripts/tools/fix-module-root-hygiene.sh`. Vedi `docs/wiki/rules/module-root-cleanup-rules.md`.
- Regola multi-repo: dopo aver modificato un modulo/tema, `cd laravel/Modules/<Nome>` (o `Themes/<Nome>`), `git remote -v`, poi **commit e push nel repo owner del modulo** — non solo nel monorepo root. Canon: `docs/wiki/rules/multi-repo-modules-themes-map.md`.
- Regola ide-helper: `cd laravel && php artisan ide-helper:models -W` e' consentito come sincronizzazione dei PHPDoc dei modelli, ma solo con lettura del diff generato, fix manuali forward-only sulle anomalie e rilancio dei quality gate dopo la wave.
- Regola contract-first PHPDoc: per relazioni trasversali come `creator`, `updater`, `deleter` i docblock devono usare `\Modules\Xot\Contracts\ProfileContract|null`, non modelli concreti come `\Modules\Meetup\Models\Profile|null`, anche se ide-helper li propone.
